...
int main(int argc, char *argv[])
{
        ...
        if(argc < 3)
        {
                printf("please give source name and dest name\n");
                return -1;
        }
        ...
        {
                if((lstat(argv[2], &st) == 0) && (S_ISDIR(st.st_mode)))
                {
                        memset(dest_name, 0, 1024);
                        last_name = strrchr(argv[1], '/');
                        if(last_name == NULL)
                                last_name = argv[1];
                        else
                                last_name++;
                        sprintf(dest_name, "%s/%s", argv[2], last_name);
                        copy_file(argv[1], dest_name);
                }else{
